...
/* ─── LocalStorage — prefixo sf_ (Spark Foco) ─── */
function loadFromStorage() {
  State.username  = localStorage.getItem('sf_username')  || '';
  State.isPremium = localStorage.getItem('sf_premium') === 'true';
...
function activatePremium(bool) {
  ...
  localStorage.setItem('sf_premium', bool ? 'true' : 'false');
}
...
async function loadFrases() {
  try {
    ...
    console.error('[Sparks Foco] Erro ao carregar frases.json:', err);
    State.frases = [
      { id: 1, text: "Foco não é fazer muitas coisas.\nÉ fazer a coisa certa agora.\nUma tarefa. Total presença." },
      { id: 2, text: "A procrastinação morre quando a clareza nasce.\nSe você não está agindo, é porque não decidiu.\nDecida. Agora." },
      { id: 3, text: "Atenção é o recurso mais escasso do século.\nQuem controla o foco controla o futuro.\nProteja o seu." }
    ];
  }
...
function updateGreeting() {
  ...
  if (el) el.textContent = `Foco total, ${State.username}.`;
}
...
async function handleShare() {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: 'Sparks Foco', text });
    } catch (_) {}
  } else {
    try {
...
function exportToCanvas(frase, premium) {
  ...
  if (premium) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Gerado no Sparks Foco', W / 2, H - 60);
  }

  /* ── Download ── */
  const link = document.createElement('a');
  link.download = `sparks-foco-${frase.id}.png`;
